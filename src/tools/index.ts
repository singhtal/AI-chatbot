import { GoogleGenAI, Content, FunctionCallingConfigMode, Type } from '@google/genai';
...
function getOrderStatus(orderId: string) {
    console.log(`Getting the order status for ${orderId}`);
...
async function callGeminiAIWithTools() {
    ...
    const context: Content[] = [
        {
            role: "user",
            parts: [{ text: "What is the status of the order 1234?" }]
        }
    ];
    ...
    const response = await googleAi.models.generateContent({
        ...
        contents: context,
        config: {
            systemInstruction: "You are a helpful assistant that gives information about the time of the day and order status",
            tools: [
                {
                    functionDeclarations: [
                        ...
                        },
                        {
                            ...
                            parameters: {
                                type: Type.OBJECT, // Use Type.OBJECT instead of 'object'
                                properties: {
                                    orderId: {
    ...
    if (functionCalls && functionCalls.length > 0) {
        ...
        if (toolCall.name === 'getOrderStatus') {
            const rawArguments = toolCall.args;

            // Access arguments directly:
            const orderId = toolCall.args?.orderId as string | undefined;
            
            if(orderId) {
                const toolResp = getOrderStatus(orderId);
